...
if (isset($_POST['create'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO products (name, brand, quantity, price, link, imageSrc) VALUES (?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("ssids", $name, $brand, $quantity, $price, $link, $imageSrc);
    if ($stmt->execute()) {
...
if (isset($_POST['update'])) {
    ...
    $stmt = $conn->prepare("UPDATE products SET brand=?, quantity=?, price=? WHERE name=?");

    // Bind the parameters
    $stmt->bind_param("sdds", $brand, $quantity, $price, $name);
